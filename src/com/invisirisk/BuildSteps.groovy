...
class BuildSteps {
    ...
    static void notifyStart(script, buildUrl) {
        try {
            ...
            def queryParams = [
                ...
                'scm_branch': gitInfo.branch,
                'scm_origin': gitInfo.origin,
                'registry_id': script.env.ECR_REGISTRY_ID,
                'region': script.env.ECR_REGION
            ]
    ...
    private static void validateVBEnvironment(script) {
        def required = [
            'VB_API_KEY',
            'VB_API_URL',
            'ECR_USERNAME',
        ...
        def missing = required.findAll { !script.env[it] }
        if (missing) {
            throw new IllegalStateException("Missing required VB environment variables: ${missing.join(', ')}")
        }
    }
